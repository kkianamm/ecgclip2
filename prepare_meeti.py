...
def main() -> None:
    ...
    ap.add_argument("--keep-unimaged", action="store_true",
                    help="keep records that have no image in labels.csv. Off by "
                         "default: MEETI ships images for only ~10k of its 784k "
                         "records, and every script here is image-based, so "
                         "imageless rows would crash evaluation.")
    ...
    for rec in tqdm(records, desc="Preparing MEETI"):
        ...
        # Every downstream script opens the image, so an imageless record is
        # unusable here -> drop it unless the user explicitly opts in. MEETI
        # ships images for only ~10k of its 784k records.
        if not ok and not args.keep_unimaged:
            continue
    ...
    print(f"\nSaved labels -> {out}")
    print(f"Rows written: {len(df)} | train {train} val {val} test {test} "
          f"(patient-disjoint by subject_id)")
    print(f"Images placed: {n_images} | records with no image: {n_missing_img}")
    if args.label_mode == "rule":
        for c in C.CLASSES:
    ...
    if n_missing_img:
        if args.keep_unimaged:
            print(f"\nWARNING: {n_missing_img} rows have no image and WILL CRASH "
                  "evaluation (--keep-unimaged was set). Remove them before running "
                  "zero_shot_eval.py / extract_features.py / finetune_clip.py.")
        else:
            print(f"\nNote: {n_missing_img} records had no image and were dropped "
                  "(MEETI ships images for only ~10k records). Pass --render-from-wfdb "
                  "to render more from MIMIC-IV-ECG waveforms, or --keep-unimaged to keep them.")
